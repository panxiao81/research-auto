...
from research_auto.models import AuthorCandidate, CrawlResult, PaperCandidate


async def crawl_track(track_url: str, *, headless: bool = True) -> tuple[CrawlResult, str]:
        ...
        for item in raw_candidates:
            ...
            paper_candidates.append(
                PaperCandidate(
                    ...
                    authors=[AuthorCandidate(name=name) for name in item.get("authors", []) if name],
                )
            )

        await browser.close()
        return CrawlResult(discovered=len(paper_candidates), paper_candidates=paper_candidates), html
...
async def _extract_accepted_papers(page) -> list[dict[str, object]]:
    ...
    for index in range(row_count):
        ...
        title = await _safe_text(modal.locator(".event-title strong").first)
        if not title:
            title = await _safe_text(trigger)
        ...
        detail_url = next((item["href"] for item in modal_links if "/details/" in item["href"]), None)
        pdf_url = next(
            (
                item["href"]
...
def crawl_track_sync(track_url: str, *, headless: bool = True) -> tuple[CrawlResult, str]:
