...
from research_auto.schema import SCHEMA_SQL
...
class Database:
        ...
        with self.connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    """,
                    (job_type, json.dumps(payload, default=str), dedupe_key, priority, max_attempts),
                )
    ...
    def list_rows(self, query: str, params: tuple[Any, ...] = ()) -> list[dict[str, Any]]:
        with self.connect() as conn:
            with conn.cursor() as cur:
